...
struct PersistenceController {
    // static allows useage throughout the entire app
    
    // shared is what the real, runtime app is using
    static let shared = PersistenceController()
    let container: NSPersistentContainer

    @MainActor
    // Get the preview for the core data, first 10
    // Useful for testing
    
    // Meant for Preview Controller
    // Spawns sample data immediately
    static let preview: PersistenceController = {
        ...
        do {
            ...
            // Note: That when Core Data optionals matter and is checked
    ...
    init(inMemory: Bool = false) {
        ...
        if inMemory {
            ...
            // Added group so both Widget and Phone App can share the CoreData
            container.persistentStoreDescriptions.first!.url = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.apyan.dex3group")!.appending(path: "Dex3.sqlite")
        }
